...
| `--init` | `InitCommand` | 配線と同時に配置。導入直後の最初のセッションから載せる（`--no-plugins`／選択 0 件でも実行） |
...
対象はどの契機でも **`common.yml` で有効な全プラグイン**（`--init` で「今回選んだ分」に限定しない）。契機に
よって置かれるものが変わると、`--init` の再実行や `SessionStart` で結果が食い違う。
