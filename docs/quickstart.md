...
対象プロジェクトのルート（`.claude` がある階層）で 3 つのファイルを用意する。

```
<プロジェクトルート>/
└── .claude/
    ├── settings.json                        ① hook で ai-harness-main を呼ぶ
    └── harness/config/
        ├── common.yml                       ② どのプラグインを有効化するか
        └── ai-harness-deny.yml              ③ そのプラグインのルール
```

### ① `.claude/settings.json` — hook を張る
...
### ② `.claude/harness/config/common.yml` — プラグインを有効化する
...
キーは DLL 名ではなく各プラグインの **`PluginName`**。書かなかったプラグインは全て無効。
...
`lib/` に DLL を足し（手順 4）、`common.yml` の `tools` に `PluginName` を追加し、その YAML を置く（手順 5）。
