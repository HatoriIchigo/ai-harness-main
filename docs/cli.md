...
引数なしで叩いても、**stdin から hook JSON が渡ってこなければ hook 経由の起動ではない**と判断し、使い方を出して
`1` で終わる。端末から直接実行した場合（stdin が端末）は stdin を読みにいかないため、入力待ちで止まることはない。
...
無アクセス 30 分でプロジェクト状態を破棄し、全プロジェクトが回収されて空になれば daemon 自体が終了する。
時間は `<実行体>/config/daemon.yml` で変えられる（起動時に読むため変更は `--restart` で反映）。
ログは `<実行体>/logs/` にのみ出力する（stderr に消費者がいないため）。
