...
- **プラグイン設定の相互検証**:
  起動検証は 2 パス。全プラグインの設定ロード後に、各プラグインが要求するファイル配置（`PluginBase.RequiredPaths`）を集めて `ValidatePeers` へ渡し、設定同士が両立しないプロジェクトを起動時に弾く。
  例＝`ai-harness-constants` が要求する定数ファイルの置き場所を `ai-harness-directory-checker` が許可していない場合（そこにファイルを作った瞬間に必ず deny される状態）。
  要求が他プラグインの許可を広げることはない。矛盾の検出にのみ使い、検出したらフェイルクローズさせる。
- **クロスプラットフォーム**:
